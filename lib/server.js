...
class GrpcServer {
  ...
  tryStart(count) {
    ...
    const bindRet = this.server.bind(`${opt.host}:${opt.port}`, grpc.ServerCredentials.createInsecure());
    this.server.start();
    this.started = this.server.started;
    if (bindRet !== 0) {
      this.app.logger.info(`[egg-grpc-server] listen ${opt.host}:${opt.port} pid:${process.pid}`);
    } else {
      this.app.logger.info(`[egg-grpc-server] bind port ${opt.port} error, retry times: ${count}`);
    }
    return bindRet;
  }
